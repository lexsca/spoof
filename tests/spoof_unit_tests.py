...
class TestHTTPRequestHandler(unittest.TestCase):
  def setUp(self):
    ...
    self.mockRequestQueue = mock.patch.object(spoof.HTTPRequestHandler,
                                              'requestReportQueue',
                                              spec=spoof.Queue.Queue).start()
    self.mockResponseQueue = mock.patch.object(spoof.HTTPRequestHandler,
  ...
  def test_Handler_reportRequestEnv_catches_Queue_Full(self):
    ...
    result = self.handler.reportRequestEnv()
  ...
  def test_Handler_sendResponse_handles_empty_response(self):
    ...
    response = self.handler.wfile.getvalue().decode(spoof.RESPONSE_ENCODING)
